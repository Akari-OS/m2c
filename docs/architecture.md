...
[Video Module]
  ...
  ▼
[Pool MCP] pool_context_get(item_id, level="L2", max_tokens=800)
  │
...
[Partner]
  ...
  │ 2. ヒットした item に対し pool_context_get(..., level="L1") で要約取得
...
## 5. MCP ツール化 — Pool との統合

> **注意（2026-06-13 更新）**: 以下は代表的なツールの抜粋です。Pool は Phase 6 以降で大幅に拡張され、
> 現在 `akari-pool-impl/crates/pool-mcp/src/server.rs` には **62 件以上** の `pool_` ツールが実装済みです。
> 最新の全ツール一覧は [`akari-pool-docs/docs/reference/mcp-tools.md`](https://github.com/Akari-OS/pool-docs/blob/main/docs/reference/mcp-tools.md) を参照してください。

これらはすべて **M2C を暗黙に経由** する代表例：

| Pool MCP Tool | M2C との関係 |
|---|---|
| `pool_add` | ファイル投入 → Analyzer 起動 → M2C MediaContext 生成 |
| `pool_cat` | item メタ情報（mediaType / analyzedAt / confidence）取得 |
| `pool_context_get` | **M2C MediaContext を Level 指定で返す（最重要）** |
| `pool_search` | L0 tags / L1 summary を全文検索 |
| `pool_list` | item 一覧（軽量） |
| `pool_update_item` | メタ情報更新（再解析トリガも含む） |
| `pool_archive` | item のアーカイブ（関連 context 含む） |
| `pool_add_relation` | item 間の関係（M2C Core には無い、Pool 拡張） |
| `pool_relations` | 関係グラフ取得 |
...
- [ ] **Consumer（Shell Module / Agent）を書く場合**
  - [ ] Pool MCP 経由で `pool_context_get(level=...)` を呼ぶ
