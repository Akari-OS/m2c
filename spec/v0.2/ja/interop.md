...
M2C XMP 名前空間（`https://github.com/Akari-OS/m2c/ns/v0.2/`）は Adobe の XMP 名前空間レジストリには登録されていない。実装はすべての XMP パケットに名前空間宣言を含めなければならない（MUST）。Adobe のレジストリへの登録は M2C が v1.0 安定版に到達した時点で推進する。
